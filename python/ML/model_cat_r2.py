...
# ==========================================
print("\n正在加载数据...")
df = pd.read_csv("CAT.csv")                # <--- 修改文件名
df.columns = ['smiles', 'CAT']             # <--- 修改目标列名
...
y = df['CAT'].iloc[valid_idx].values       # <--- 修改目标列名
...
output_dir = "ml_model_CATmodel_cat"           # <--- 修改输出目录名
...
# 注意：train_sub_indices, val_indices, test_indices 已经是原始数据索引（来自 valid_idx 的索引）
# 因为 train_test_split 返回的索引是原始数据的索引，所以可以直接用它们从 valid_smiles 中取值。
# 但需要注意：train_sub_indices 是原始数据中的索引（即原始 df 中的行号），而 valid_smiles 列表是按照 valid_idx 顺序排列的，
# 所以我们需要将原始索引映射到 valid_smiles 中的位置。valid_smiles 的顺序与 valid_idx 对应，因此需要构建一个映射。
# 构建原始索引到 valid_smiles 位置的映射
orig_to_valid_pos = {orig_idx: pos for pos, orig_idx in enumerate(valid_idx)}

# 获取训练集、验证集、测试集在 valid_smiles 中的位置
...
train_df = pd.DataFrame({'SMILES': train_smiles, 'True_CAT': train_true})   # <--- 修改列名
...
val_df = pd.DataFrame({'SMILES': val_smiles, 'True_CAT': val_true})
...
test_df = pd.DataFrame({'SMILES': test_smiles, 'True_CAT': test_true})
