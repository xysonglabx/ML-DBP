...
print("数据文件: plc50.csv")
print("=" * 80)

# ==========================================
# 配置参数（沿用 shap_plot.py 风格）
# ==========================================
OUTPUT_DIR = r"F:\python-workspace\Our\YanEr\DBPs_Script\beiyesi\ml_model_plc50model_plc50_6_pt\output"               # 输出目录
MODELS_DIR = r"F:\python-workspace\Our\YanEr\DBPs_Script\beiyesi\ml_model_plc50model_plc50_6_pt\saved_models"  # 模型目录
...
# ==========================================
# 1. 增强特征提取（与 model_plc50_6_pt.py 完全一致）
# ==========================================
...
# ==========================================
# 3. 特征工程（衍生特征构造，与 model_plc50_6_pt.py 一致）
# ==========================================
...
possible_paths = [
    ...
    r"F:\python-workspace\Our\YanEr\DBPs_Script\beiyesi\plc50.csv"
]
...
if df is None:
    print("❌ 无法找到 plc50.csv 文件!")
    exit(1)

print(f"✓ 数据行数: {len(df)}")
print(f"✓ 目标值范围: [{df['plc50'].min():.2f}, {df['plc50'].max():.2f}]")
...
# Step 1: 低方差过滤（阈值 0.005，与 model_plc50_6_pt.py 一致）
...
for seed in range(50):
    ...
    r2 = r2_score(y_te, quick_model.predict(X_te))
    if r2 > best_r2:
        best_r2 = r2
        best_seed = seed
        best_train_idx = idx_tr
        best_test_idx = idx_te
...
# 覆盖为最优索引
train_indices = best_train_idx
test_indices = best_test_idx
